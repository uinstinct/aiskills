...
The script deletes `agents.md/<name>/` (no-op with stderr warning if the folder is already missing) and removes the entry from `mapping.yml`'s `installed_agents_md` section.
