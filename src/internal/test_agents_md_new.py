"""Unit tests for src/internal/agents_md_new.py.
...
class AgentsMdNewTests(unittest.TestCase):
    def setUp(self):
        ...
        self._patches = [
            ...
            patch.object(agents_md_new, "repo_root", return_value=self.root),
        ]
    ...
    def _run(
        ...
        with patch.object(
            ...
            return agents_md_new.main(argv)

    def test_creates_integration_with_all_fields(self):
        rc = self._run(
            ["my-snippet"],
            inputs=["a fine snippet", "0.2.1", "1,4"],
        )
        self.assertEqual(rc, 0)
        entry_dir = self.root / "assets" / "agents.md" / "my-snippet"
        self.assertTrue((entry_dir / "snippet.md").is_file())
        self.assertIn(
            "TODO: describe the integration",
            (entry_dir / "snippet.md").read_text(encoding="utf-8"),
        )
        manifest_text = (entry_dir / "manifest.yml").read_text(encoding="utf-8")
        self.assertIn("name: my-snippet", manifest_text)
        self.assertIn("description: a fine snippet", manifest_text)
        self.assertIn("version: 0.2.1", manifest_text)
        self.assertIn("claude-code", manifest_text)
        self.assertIn("codebuff", manifest_text)
        self.assertIn("snippet_file: snippet.md", manifest_text)

        mapping_text = (self.root / "mapping.yml").read_text(encoding="utf-8")
        self.assertIn("name: my-snippet", mapping_text)
        self.assertIn("source_url: local", mapping_text)
        self.assertIn("install_path: assets/agents.md/my-snippet/", mapping_text)

    def test_default_version_used_on_empty_input(self):
        rc = self._run(["alpha"], inputs=["desc here", "", ""])
        self.assertEqual(rc, 0)
        manifest_text = (
            self.root / "assets" / "agents.md" / "alpha" / "manifest.yml"
    ...
    def test_invalid_semver_reprompts(self):
        ...
        self.assertEqual(rc, 0)
        manifest_text = (
            self.root / "assets" / "agents.md" / "beta" / "manifest.yml"
        ).read_text(encoding="utf-8")
    ...
    def test_empty_description_reprompts(self):
        ...
        self.assertEqual(rc, 0)
        manifest_text = (
            self.root / "assets" / "agents.md" / "gamma" / "manifest.yml"
        ).read_text(encoding="utf-8")
        self.assertIn("description: real description", manifest_text)

    def test_name_sanitized(self):
        rc = self._run(["My Cool Snippet!"], inputs=["d", "", ""])
        self.assertEqual(rc, 0)
        self.assertTrue(
            (self.root / "assets" / "agents.md" / "my-cool-snippet").is_dir()
        )

    def test_collision_without_force_raises(self):
        (self.root / "assets" / "agents.md" / "dup").mkdir()
        with patch.object(
            sys.stdin, "isatty", return_value=True
        ), self.assertRaises(SystemExit) as ctx:
            agents_md_new.main(["dup"])
        self.assertNotEqual(ctx.exception.code, 0)

    def test_force_overwrites_existing(self):
        existing = self.root / "assets" / "agents.md" / "dup"
        existing.mkdir()
        (existing / "old.txt").write_text("old", encoding="utf-8")
        rc = self._run(["dup", "--force"], inputs=["fresh description", "", ""])
        self.assertEqual(rc, 0)
        self.assertFalse(
            (self.root / "assets" / "agents.md" / "dup" / "old.txt").exists()
        )
        self.assertTrue(
            (self.root / "assets" / "agents.md" / "dup" / "snippet.md").is_file()
        )
