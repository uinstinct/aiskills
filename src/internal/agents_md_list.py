...
from lib import AGENTS_MD_KEY, IngestError, fail, load_mapping


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="agents_md_list",
        description="Print every agents.md integration currently in the registry.",
    )
    parser.add_argument(
        "--json",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.json:
            ...
            return 0
        if not entries:
            print("(no agents.md integrations in registry)")
            return 0
